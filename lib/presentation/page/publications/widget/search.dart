...
/// [FlabrSearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [FlabrSearchDelegate] can only be associated with one active [showFlabrSearch]
/// call. Call [FlabrSearchDelegate.close] before re-using the same delegate instance
/// for another [showFlabrSearch] call.
///
...
/// The animation provided by [FlabrSearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [FlabrSearchDelegate] to define the content of the search page.
Future<T?> showFlabrSearch<T>({
  required BuildContext context,
  required FlabrSearchDelegate<T> delegate,
  String? query = '',
...
/// Delegate for [showFlabrSearch] to define the content of the search page.
///
/// The search page always shows an [AppBar] at the top where users can
/// enter their search queries. The buttons shown before and after the search
/// query text field can be customized via [FlabrSearchDelegate.buildLeading]
/// and [FlabrSearchDelegate.buildActions]. Additionally, a widget can be placed
/// across the bottom of the [AppBar] via [FlabrSearchDelegate.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [FlabrSearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [FlabrSearchDelegate.buildResults].
///
/// [FlabrSearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [FlabrSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [FlabrSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [FlabrSearchDelegate.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [showFlabrSearch] about the selected search result.
///
/// A given [FlabrSearchDelegate] can only be associated with one active [showFlabrSearch]
/// call. Call [FlabrSearchDelegate.close] before re-using the same delegate instance
/// for another [showFlabrSearch] call.
...
/// ## Handling emojis and other complex characters
/// {@macro flutter.widgets.EditableText.onChanged}
abstract class FlabrSearchDelegate<T> {
  ...
  /// communicate the result back to the initial caller of [showFlabrSearch].
  ...
  /// to [showFlabrSearch] that launched the search initially.
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [FlabrSearchDelegate.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [FlabrSearchDelegate.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final FlabrSearchDelegate<T> delegate;
