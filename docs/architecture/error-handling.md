...
1. непустой `message` из `AppException`;
2. сообщение для известного типа исключения или непустой строки;
3. сообщение для `AppFailureType`;
4. общее сообщение о неизвестной ошибке.
